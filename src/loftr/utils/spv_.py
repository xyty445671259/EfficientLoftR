...
from .geometry import warp_kpts,estimate_transform,apply_transform
...
def spvs_coarse(data, config):
    ...
    annotated_pts0 = data['annotated0'].cpu().numpy().astype(np.float32)
    annotated_pts1 = data['annotated1'].cpu().numpy().astype(np.float32)
    M_0to1 = torch.tensor(estimate_transform(annotated_pts0, annotated_pts1),dtype=torch.float32).cuda()
    M_1to0 = torch.inverse(M_0to1).cuda()
    w_pt0_i = apply_transform(grid_pt0_i, M_0to1)
    w_pt1_i = apply_transform(grid_pt1_i, M_1to0)
...
def spvs_fine(data, config, logger = None):
    ...
    if m == 0:  # special case: there is no coarse gt
        ...
        for b in range(N):
            ...
            match = int(mask.sum())
            M_0to1 = torch.tensor(estimate_transform(annotated_pts0, annotated_pts1), dtype=torch.float32).cuda()
            correct_0to1_f_mask, w_pt0_i_mask = apply_transform(grid_pt0_f_unfold[mask].reshape(1,-1,2), M_0to1, H1, W1, 'fine')
